...
const bot = async () => {
  ...
  const Alpha = async () => {
    try {
      ...
      const { lavss } = (await axios(`https://raw.githubusercontent.com/${config.REPO}/main/package.json`)).data;
          ...
          if (statusCode !== DisconnectReason.loggedOut) {
            ...
            switch (statusCode) {
              ...
              case DisconnectReason.connectionReplaced:
                console.log("🔄 Connection replaced, a new session is opened and reconnected...");
                Alpha();
                break;
      ...
      conn.ev.on("messages.upsert", async (m) => {
        ...
        if (m.messages[0]?.messageStubType) {
          ...
          const groupMetadata = await conn.groupMetadata(jid).catch((e) => ({ participants: [] }));
          const admins = (jid) =>
          ...
          if (m.messages[0].messageStubType === proto.WebMessageInfo.StubType.GROUP_PARTICIPANT_DEMOTE) {
            ...
            if (antipromote === "true" && groupMetadata.owner !== actor && botJid !== actor &&
              admins(botJid) && !botadmins.map((j) => j + "@s.whatsapp.net").includes(actor) &&
              admins(actor) && admins(participant)) {
              await conn.groupParticipantsUpdate(jid, [actor], "demote");
              await sleep(100);
        ...
        plugins.commands.map(async (command) => {
          ...
          try {
            ...
            await conn.sendMessage(m.user.jid, {
              text: `*_ERROR REPORT_*\n\n` +
                `\`\`\`command: ${m.command}\`\`\`\n` +
                `\`\`\`version: ${require("./package.json").version}\`\`\`\n` +
                `\`\`\`latest version: ${version}\`\`\`\n` +
                `\`\`\`user: @${m.participant.replace(/[^0-9]/g, "")}\`\`\`\n\n` +
                `\`\`\`message: ${m.body}\`\`\`\n` +
                `\`\`\`error: ${require("util").format(errr)}\`\`\``,
              mentions: [m.sender],
            }, { quoted: m.data });
            console.error('COMMAND ERROR:', errr);
          }
        });
