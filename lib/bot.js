...
const bot = async () => {
  ...
  const Alpha = async () => {
    try {
      ...
      const { lavss } = (await axios(`https://raw.githubusercontent.com/${config.REPO}/main/package.json`,)).data.version
      console.log(lavss)
      ...
      conn.ev.on("messages.upsert", async (m) => {
        const { pdm, antipromote, antidemote, antibot, antilink, antiword } = await groupDB(
          ["pdm", "antidemote", "antipromote","antibot", "antilink", "antiword"],
