    ...
    try {
      ...
      conn.ws.on("CB:call", async (json) => {
        if (json.content[0].tag == "offer") {
          ...
          console.log("☎️ Received call from " + callfrom);
          const blockMessage = "Sorry, no calls. Please use Text or Voice Message\n> automated System\n> You have been blocked for this.";
          const rejectMessage = "Sorry, no calls. Please use Text or Voice Message\n> automated System";
      ...
      conn.ev.on("messages.upsert", async (m) => {
        const { pdm, antipromote, antidemote } = await groupDB(
          ["pdm", "antidemote", "antipromote"],
