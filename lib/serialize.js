...
async function serialize(msg, conn) {
  ...
  if (msg.key) {
    ...
    try {
      msg.sudo = config.SUDO.split(",").includes(parsedJid(msg.sender)[0].split("@")[0]);
    } catch {
      msg.sudo = false;
    }
  }
  ...
  if (msg.message) {
          ...
          msg.quoted = {
            ...
            message:
              type === "viewOnceMessageV2"
                ? quoted.quotedMessage.ephemeralMessage.message.viewOnceMessageV2.message
    ...
    try {
      ...
      if (msg.type === "interactiveResponseMessage") {
        let msgg = msg.message[msg.type] || m.msg;
        if (msgg.nativeFlowResponseMessage) {
          let { id } = JSON.parse(msgg.nativeFlowResponseMessage.paramsJson) || {};
          msg.body = id;
        }
      }
    } catch (error) {
