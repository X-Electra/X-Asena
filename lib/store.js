...
function bind(conn) {
  ...
  async function loadMessages(jid, count, cursor) {
    const list = assertMessageList(jid);
  ...
  async function mostRecentMessage(jid) {
    var _a;
    let message =
  ...
  async function fetchImageUrl(jid) {
  ...
  async function fetchBroadcastListInfo(jid) {
    if (!groupMetadata[jid]) {
      groupMetadata[jid] = await (conn === null || conn === void 0
        ? void 0
  ...
  async function fetchMessageReceipts({ remoteJid, id }) {
